...
def train_riskgraph_model(data_dir: str = "outputs", model_dir: str = "models", cost_fp: float = 10.0, cost_fn: float = 120.0):
    print("=" * 60)
    print("STAGE 8 & 9: TRAINING RISKGRAPH XGBOOST (+ CALIBRATION & METRICS)")
    print("=" * 60)
    
    os.makedirs(model_dir, exist_ok=True)
    
    # 1. Load Parquet Data
    print("Loading graph-enriched datasets...")
    train_df = pd.read_parquet(os.path.join(data_dir, "train_graph.parquet"))
    val_df = pd.read_parquet(os.path.join(data_dir, "val_graph.parquet"))
    test_df = pd.read_parquet(os.path.join(data_dir, "test_graph.parquet"))
    
    # 2. Select All 18 Features
    features = [
    ...
    # 4. Train RiskGraph XGBoost Model
    print("\nTraining RiskGraph XGBoost model...")
    model = XGBClassifier(
        n_estimators=300,
        max_depth=6,
        learning_rate=0.05,
    ...
    print(f"\nSaved RiskGraph model to {model_path}")
    ...
    calibrator_path = os.path.join(model_dir, "riskgraph_calibrator.pkl")
    joblib.dump(calibrator, calibrator_path)
    print(f"Saved RiskGraph Calibrator to {calibrator_path}")
    
    # 6. Optimize Threshold on Calibrated Validation Set
    optimal_thresh = find_optimal_threshold(calibrator, val_raw_probs, y_val, cost_fp, cost_fn)
    print(f"Optimal RiskGraph Threshold (Calibrated Val Set): {optimal_thresh:.2f}")
    
    # 7. Evaluate Split Function
    def evaluate_split(name, X, y, threshold):
        ...
        print(f"\n--- RISKGRAPH EVALUATION: {name.upper()} (Threshold={threshold:.2f}) ---")
    ...
    with open(os.path.join(model_dir, "riskgraph_metrics.json"), "w") as f:
        json.dump(metrics_summary, f, indent=4)

if __name__ == "__main__":
    train_riskgraph_model()